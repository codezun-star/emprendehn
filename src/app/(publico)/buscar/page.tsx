...
export default async function PaginaBuscar({ searchParams }: PageProps<"/buscar">) {
  ...
  // Sin texto: la página canónica es la de categoría (o categoría + ciudad).
  if (!q && categoria && pagina === 1) {
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <div className="rounded-2xl bg-brand-dark p-3">
        <Buscador
          ...
          valores={{ q, categoria: categoria?.slug, ciudad: ciudad?.slug }}
          compacto
        />
      </div>
      ...
      {negocios.length > 0 ? (
        ...
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-brand-dark/10">
          ...
          <p className="mt-2 text-sm text-ink/70">
            Prueba con otras palabras, quita filtros o{" "}
